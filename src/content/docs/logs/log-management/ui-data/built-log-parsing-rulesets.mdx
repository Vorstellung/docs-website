...
New Relic can [parse common log formats](/docs/logs/log-management/ui-data/logs-parsing-built-rules-custom-parsing) according to built-in rulesets, so that you don't have to [create your own parsing rules](/docs/logs/log-management/ui-data/logs-parsing-built-rules-custom-parsing#custom-parsing). Here we present each log parsing ruleset, their [Grok](https://grokdebug.herokuapp.com/patterns# "Link opens in a new window.") patterns, and what fields are parsed.

To enable built-in log parsing, see [How to add the `logtype` attribute](/docs/logs/log-management/ui-data/logs-parsing-built-rules-custom-parsing#logtype).
